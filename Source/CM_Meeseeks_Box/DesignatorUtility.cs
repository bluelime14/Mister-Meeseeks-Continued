...
namespace CM_Meeseeks_Box
{
    public static class DesignatorUtility
    {
        ...
        public static void ForceAllDesignationsOnCell(IntVec3 cell, Map map)
        {
            ...
            if (cachedDesignators == null)
            {
                ...
                cachedDesignators = allDesignationCategoryDefsForReading.SelectMany(category => category.ResolvedAllowedDesignators)
                                                                        ...
                                                                        .Where(designator => !designator.GetType().Name.StartsWith("Designator_Area") &&
            ...
            // First pull out all existing designations on this cell
            List<Designation> existingHere = map.designationManager.AllDesignations.Where(d => d.target.Cell == cell).ToList();
            foreach (Designation designation in existingHere)
            {
                ...
                else if (designationDef.targetType == TargetType.Thing)
                {
                    foreach (Thing thing in cell.GetThingList(map))
                    {
                        if (designator.CanDesignateThing(thing).Accepted)
                            map.designationManager.AddDesignation(new Designation(thing, designationDef));
                    }
                }
            }
        }
        ...
        public static void RestoreDesignationsOnCell(IntVec3 cell, Map map)
        {
            ...
            // Take off designations on cell, but also designations on buildings that intersect the cell
            List<Thing> thingsHere = cell.GetThingList(map);
            List<Designation> toRemove = map.designationManager.AllDesignations
                .Where(designation => designation.target.Cell == cell || (designation.target.HasThing && thingsHere.Contains(designation.target.Thing)))
                .ToList();

            foreach (Designation designation in toRemove)
            {
                map.designationManager.RemoveDesignation(designation);
            }

            foreach (Designation designation in savedDesignations)
            {
