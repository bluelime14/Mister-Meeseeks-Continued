...
namespace CM_Meeseeks_Box
{
    public static class MeeseeksUtility
    {
        ...
        // WorkGiver_Grower.wantedPlantDef is a *static* shared by every grower scan.
        // Vanilla's PotentialWorkCellsIn loop sets it per zone and clears it afterwards,
        // and JobOnCell only recalculates it when it is null. Because we call
        // HasJobOnCell/JobOnCell directly, we would otherwise inherit whatever plant the
        // last scan left behind and sow that instead of the one this zone actually wants.
        // Clear it before and after so each call resolves against its own cell and we
        // leave the field in the null state vanilla expects between scans.
        private static readonly FieldInfo wantedPlantDefField = AccessTools.Field(typeof(WorkGiver_Grower), "wantedPlantDef");

        private static void ClearWantedPlantDef()
        {
            if (wantedPlantDefField != null)
                wantedPlantDefField.SetValue(null, null);
        }

        public static bool HasJobOnCellSafe(this WorkGiver_Scanner scanner, Pawn pawn, IntVec3 cell, bool forced = true)
        {
            ClearWantedPlantDef();
            try { return scanner.HasJobOnCell(pawn, cell, forced); }
            finally { ClearWantedPlantDef(); }
        }

        public static Verse.AI.Job JobOnCellSafe(this WorkGiver_Scanner scanner, Pawn pawn, IntVec3 cell, bool forced = true)
        {
            ClearWantedPlantDef();
            try { return scanner.JobOnCell(pawn, cell, forced); }
            finally { ClearWantedPlantDef(); }
        }
        ...
        public static void SpawnMeeseeks(Pawn creator, ThingWithComps creatingThing, Map map)
        {
            ...
            MeeseeksUtility.PlayPoofInSound(mrMeeseeksLookAtMe);
