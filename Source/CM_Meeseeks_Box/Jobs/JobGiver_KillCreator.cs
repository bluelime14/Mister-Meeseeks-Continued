    ...
    public class JobGiver_KillCreator : ThinkNode_JobGiver
    {
        ...
        protected override Job TryGiveJob(Pawn pawn)
        {
            ...
            if (job == null)
            {
                ...
                {
                    using (PawnPath pawnPath = pawn.Map.pathFinder.FindPathNow(pawn.Position, target.Position, TraverseParms.For(pawn, Danger.Deadly, TraverseMode.PassDoors)))
                    {
                        if (!pawnPath.Found)
                        {
        ...
        private Job TryGetEquipmentJob(Pawn pawn, Thing target)
        {
            ...
            {
                if (pawn.CanReach(selectedEquipment, PathEndMode.Touch, Danger.Deadly, canBashDoors: true))
                {
                    using (PawnPath pawnPath = pawn.Map.pathFinder.FindPathNow(pawn.Position, selectedEquipment, TraverseParms.For(pawn, Danger.Deadly, TraverseMode.PassDoors)))
                    {
                        if (!pawnPath.Found)
                        {
