    ...
    public class CompMeeseeksMemory : ThingComp
    {
        ...
        private TargetIndex GetJobPrimaryTarget(Job job)
        {
            ...
            if (job.workGiverDef != null && job.workGiverDef.Worker != null)
            {
                ...
                if (workGiverScanner != null)
                {
                    // Designation-driven workgivers (CutPlantDesignated, HarvestDesignated, mining,
                    // etc.) only return a job when a designation is present on the target. By the
                    // time the job actually starts, the temporary designations put down while the
                    // float menu was open have already been restored, so every check here failed
                    // and we fell through to "default to any target". The same fudge the toil check
                    // uses makes DesignationOn/DesignationAt answer during this lookup, so the real
                    // primary target can be identified instead of guessed.
                    bool previousFudge = DesignatorUtility.getFudgedForToilCheck;
                    DesignatorUtility.getFudgedForToilCheck = true;
                    try
                    {
                    ...
                    {
                        DesignatorUtility.getFudgedForToilCheck = previousFudge;
                    }
                }
                else
                {
        ...
        private bool HasSameJobOnCell(Job job, WorkGiver_Scanner workGiverScanner, LocalTargetInfo targetInfo)
        {
            if (HasValidTarget(targetInfo) && !targetInfo.HasThing)
            {
                Job getJob = workGiverScanner.JobOnCellSafe(Meeseeks, targetInfo.Cell, true);
                if (getJob != null && getJob.def == job.def)
                    return true;
            }
