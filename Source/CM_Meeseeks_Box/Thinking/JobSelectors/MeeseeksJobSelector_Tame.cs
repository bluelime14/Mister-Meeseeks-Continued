...
namespace CM_Meeseeks_Box
{
    public class MeeseeksJobSelector_Tame : MeeseeksJobSelector
    {
        ...
        {
            float minNutrition = JobDriver_InteractAnimal.RequiredNutritionPerFeed(tamee) * 2f * 4f;
            ThingDef foodDef;
            Thing thing = FoodUtility.BestFoodSourceOnMap(pawn, tamee, false, out foodDef, FoodPreferability.RawTasty, allowPlant: false, allowDrug: false, allowCorpse: false, allowDispenserFull: false, allowDispenserEmpty: false, minNutrition: minNutrition);
            if (thing == null)
            {
                return null;
            }
